...
export default function TopBar({ user, onOpenSidebar }) {
  ...
  return (
    <header className="lg:hidden bg-horizon-card/80 backdrop-blur-sm border-b border-horizon px-4 py-2 shadow-sm sticky top-0 z-30">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button
            onClick={onOpenSidebar}
            className="flex items-center justify-center w-11 h-11 rounded-lg text-horizon-text hover:bg-horizon-card transition-colors cursor-pointer"
            aria-label="פתח תפריט ניווט"
          >
            <Menu className="w-5 h-5" />
          </button>
          <h1 className="text-base font-bold text-horizon-text truncate">
            {pageTitle}
          </h1>
        </div>
