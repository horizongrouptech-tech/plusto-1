  ...
  const handleRecalculatePerformance = async () => { // <--- שינוי שם הפונקציה כאן
    ...
    try {
        ...
        if (financialManagerEmails.length === 0) {
            toast.info('לא נמצאו מנהלי כספים במערכת לחישוב ביצועים.');
    ...
    try {
      ...
      if (result.success && result.recommendations && result.recommendations.length > 0) {
        ...
        setTimeout(() => {
          ...
          toast.success("המלצות משודרגות נוצרו בהצלחה!");
        }, 1000);
      } else {
  ...
  const handleUpgradePromptSubmit = async (userPrompt) => {
    ...
    try {
      ...
      if (upgradedRec && upgradedRec.title) {
        ...
        toast.error("הבינה המלאכותית לא החזירה המלצה משודרגת. נסה הנחיה אחרת.");
      }
    } catch (error) {
