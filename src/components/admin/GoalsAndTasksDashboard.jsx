  ...
  const handleDeleteTask = async (taskId, taskName) => {
    ...
    if (subtasks.length > 0) {
      ...
      try {
        ...
        toast.success(deleteSubtasks ? 'היעד וכל התת-משימות נמחקו בהצלחה' : 'היעד נמחק והתת-משימות הפכו לעצמאיות');
      } catch (error) {
...
function EditTaskModal({ isOpen, onClose, task, currentUser, allGoals, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!editedTask.start_date || !editedTask.end_date) {
      toast.warning('יש להזין תאריכי התחלה וסיום');
      return;
    }
...
function CreateGoalModal({ isOpen, onClose, customer, currentUser, existingGoalsCount, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name.trim()) {
      toast.warning('יש להזין שם ליעד');
      return;
    }
